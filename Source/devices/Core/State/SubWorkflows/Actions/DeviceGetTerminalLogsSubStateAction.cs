...
{
    internal class DeviceGetTerminalLogsSubStateAction : DeviceBaseSubStateAction
    {
        public override DeviceSubWorkflowState WorkflowStateType => GetTerminalLogs;

        public DeviceGetTerminalLogsSubStateAction(IDeviceSubStateController _) : base(_) { }
        ...
        public override async Task DoWork()
        {
            ...
            {
                //_ = Controller.LoggingClient.LogErrorAsync("Unable to find a state object while attempting to obtain terminal logs.");
                Console.WriteLine("Unable to find a state object while attempting to obtain terminal logs.");
                _ = Error(this);
            }
            else
            {
                ...
                foreach (LinkActionRequest linkActionRequest in linkRequest.Actions)
                {
                    ...
                    if (cardDevice != null)
                    {
                        ...
                        {
                            //_ = Controller.LoggingClient.LogErrorAsync($"Unable to process VIPA DUMP LOGS request from device - '{Controller.DeviceEvent}'.");
                            Console.WriteLine($"Unable to process VIPA DUMP LOGS request from device - '{Controller.DeviceEvent}'.");
